...
<?php
    if(empty($_GET['idx'])){
        echo "<script>alert('존재하지 않는 게시글 번호입니다.');location.href='./research.php'</script>";
        exit();
    }
    $idx = $_GET['idx'];
    ...
    if($result == false){
        echo "<script>alert('서버에 예기친 못한 에러가 발생했습니다. 관리자께 문의하세요.');location.href='./research.php'</script>";
        exit();
    }
...
?>
    <form action="action.php" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-floating">
            <textarea class="form-control" placeholder="간략 내용" id="floatingTextarea2" style="height: 300px" name="content" minlength="1" maxlength="300"><?php echo $row['description'];?></textarea>
        </div>
        <br>
        <div class="mb-3"> 
            <?php 
                echo "<a href='./research_file_download.php?idx={$row['idx']}&file={$row['file_real']}'>{$row['file_real']}</a>";
            ?>
        </div>
        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
            <a class="btn btn-primary" href="./research.php" role="button">뒤로</a>
            <?php
                if($_SESSION['class_number'] == $row['class_number']){
                    echo "<a class='btn btn-primary' href='./research_modify.php?idx={$_GET['idx']}' role='button'>수정</a>";
                    echo "<a class='btn btn-primary' href='./action.php?mode=delete&write_page=research&idx={$_GET['idx']}&csrf_token={$csrf_token}' role='button'>삭제</a>";
                }
            ?>
